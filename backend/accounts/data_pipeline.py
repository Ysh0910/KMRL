...
class Pipline(APIView):
    ...
    # Configuration from the user's JSON input
        ...
        def transform_train_data(data):
            ...
            fitness = flatten(data["fitness_certificates"])
            job_cards = flatten(data["job_card_status"])
            branding = flatten(data["branding_priorities"])
            mileage = flatten(data["mileage"])

            result = []
            for train_id in fitness.keys():  # assume all have same train_ids
                row = {
                    "train_id": train_id,
                    "fitness_certificates": fitness.get(train_id),
                    "job_card_status": job_cards.get(train_id),
                    "branding_priority": branding.get(train_id),
                    "mileage": mileage.get(train_id),
                }
                result.append(row)
        ...
        def send_to_ml_api(data):
            try:
                ...
                response = requests.get('http://localhost:8000/postmodeldata').json()
                print(response)   # already dict or list
